...
const { VUE2_TYPE } = require("./constant");

const { compile, writeToFile, createDirSync } = require("./utils");
...
module.exports = class Action extends Logger {
  ...
  async addComponent(name, dest, type) {
    const ejsFile =
      type === VUE2_TYPE ? "vue2-component.ejs" : "vue3-component.ejs";
  ...
  async addStore(name, dest, type) {
    ...
    if (!vue2Flag)
      this.handleEjsToFile(targetDest, "vue3-types.ejs", {}, typesFileName);
  }

  // 添加文件的action
  async addFile(name, dest) {
    const data = {
      date: new Date().getFullYear(),
    };
    const targetDest = path.resolve(dest);
    const ejsFile = `${name}.ejs`;
    const fileName = `${name}`;
    this.handleEjsToFile(targetDest, ejsFile, data, fileName);
  }
};
