...
const addStoreAction = async (name, dest, type) => {
  const fileExt = type === VUE2_ACTION ? "js" : "ts"
  const ejsFile = type === VUE2_ACTION ? "vue2-store.ejs" : "vue3-store.ejs"
  // 1. 编译ejs模块 result
  const storeResult = await compile(ejsFile, {})

  // 2. 写入文件的操作
  const targetStorePath = path.resolve(dest, `index.${fileExt}`)
  writeToFile(targetStorePath, storeResult)

  if (type === VUE3_ACTION) {
    const typesResult = await compile("vue3_types.ejs", {})
    const targetTypesPath = path.resolve(dest, `types.${fileExt}`)
