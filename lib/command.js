...
const { KV_CLI, VUE2_TYPE, VUE3_TYPE } = require("./constant");
...
module.exports = class Command extends Logger {
  ...
  vue2cpn() {
    this.program
      ...
      .description(
        "add vue2 component, eg: kv vue2-cpn HelloWord [-d src/components/common]"
      )
      .action((name) => {
        this.action.addComponent(
          name,
          program.dest || "src/components/common",
          VUE2_TYPE
        );
      });
  }
  ...
  vue2router() {
    this.program
      ...
      .description(
        "add vue2 router config, eg: kv vue2-router Home [-d src/pages]"
      )
      .action((name) => {
        this.action.addRouter(
          name,
          program.dest || `src/router/map`,
          VUE2_TYPE
        );
      });
  }
  ...
  vue2store() {
    this.program
      ...
      .description(
        "add vue2 store, eg: kv vue2-store Home [-d src/store/modules]"
      )
      .action((name) => {
        this.action.addStore(
          name,
          program.dest || `src/store/modules`,
          VUE2_TYPE
        );
      });
  }
  ...
  vue3cpn() {
    this.program
      ...
      .description(
        "add vue3 component, eg: kv vue3-cpn HelloWord [-d src/components/common]"
      )
      .action((name) => {
        this.action.addComponent(
          name,
  ...
  vue3router() {
    this.program
      ...
      .description(
        "add vue3 router config, eg: kv vue3-router Home [-d src/views]"
      )
      .action((name) => {
        this.action.addRouter(
          name,
  ...
  vue3store() {
    this.program
      ...
      .description(
        "add vue3 store, eg: kv vue3-store Home [-d src/store/modules]"
      )
      .action((name) => {
        this.action.addStore(
          name,
  ...
  add() {
    this.program
      .command("add <name>")
      .description("add file, eg: kv add LICENSE [-d src]")
      .action((name) => {
        this.action.addFile(name, program.dest || `src`);
      });
  }

  register(cmd) {
    if (
      this.commands.some((key) => {
        key === cmd;
      })
    ) {
      this.red(
        `The command ${cmd} already exists. Please overwrite the command action method implement directly.`
      );
    } else {
